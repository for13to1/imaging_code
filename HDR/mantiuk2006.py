...
class Mantiuk2006:
    ...
    def _modify_contrast_equalization(self, R_list: List[Tuple[np.ndarray, np.ndarray]]) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    def _solve_optimization(self, B: np.ndarray, pyramid_shapes: List[Tuple[int, int]], 
                           p_list: List[Tuple[np.ndarray, np.ndarray]], x0: np.ndarray) -> np.ndarray:
        ...
        X_res, info = bicgstab(A, B.flatten(), x0=x0.flatten(), rtol=self.solver_tol, maxiter=self.solver_maxiter)
